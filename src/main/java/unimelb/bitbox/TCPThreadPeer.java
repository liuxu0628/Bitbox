 ...
 */
public class TCPThreadPeer extends Thread implements Runnable {
     ...
     */
    public TCPThreadPeer(Socket socket, FileSystemManager fileSystemManager) {
        this.socket = socket;
    ...
    public synchronized void run() {
        ...
        try {
            ...
            while (!socket.isClosed()) {
                ...
                if (TCPPeer.getEvent().size() > count) {
                    FileSystemEvent eve = TCPPeer.getEvent().get(count);
                    switch (eve.event) {
                        case FILE_CREATE:
                            TCPFileCreate file_Cre = new TCPFileCreate(socket);
                        ...
                        case FILE_DELETE:
                            ...
                            TCPFileDelete file_Del = new TCPFileDelete(socket);
                        ...
                        case FILE_MODIFY:
                            // System.out.println("Connection closed: " + socket.isClosed());
                            TCPFileModify file_Mod = new TCPFileModify(socket);
                        ...
                        case DIRECTORY_CREATE:
                            TCPDirectoryCreate dir_Cre = new TCPDirectoryCreate(socket);
                        ...
                        case DIRECTORY_DELETE:
                            TCPDirectoryDelete dir_Del = new TCPDirectoryDelete(socket);
                ...
                if (TCPPeer.getsyn_Event().size() > syn) {
                    System.out.println("Syn detected");
//                    System.out.println(syn);
                    // System.out.println(Peer.getsyn_Event());
//                    System.out.println();
                    FileSystemEvent eve = TCPPeer.getsyn_Event().get(syn);
                    if (!eve.event.toString().equals("DIRECTORY_CREATE")) {
                        ...
                        TCPDirectoryCreate dir_Dir = new TCPDirectoryCreate(socket);
                ...
                if (input.ready()) {
                    ...
                    switch (command) {
                        ...
                        case "FILE_DELETE_REQUEST":
                            new TCPFileDelete(socket).FileDelete_response(request, fileSystemManager);
                        ...
                        case "DIRECTORY_CREATE_REQUEST":
                            new TCPDirectoryCreate(socket).DirectoryCreate_response(request, fileSystemManager);
                        ...
                        case "DIRECTORY_DELETE_REQUEST":
                            new TCPDirectoryDelete(socket).DirectoryDelete_response(request, fileSystemManager);
...
//                            }
                        ...
                        case "FILE_CREATE_REQUEST":
                            try {
                                if (new TCPFileCreate(socket).FileCreate_response(request, fileSystemManager)) {
                                    FILE_BYTETO.add(md5);
                                }
                        ...
                        case "FILE_MODIFY_REQUEST":
                            if (new TCPFileModify(socket).FileModify_response(request, fileSystemManager)) {
                                FILE_BYTETO.add(md5);
                            }
                        ...
                        case "FILE_BYTES_REQUEST":
                            if (FILE_BYTE.contains(md5)) {
                                ...
                                if (new TCPFileBytes(socket).FileBytes_response(request, fileSystemManager)) {
                                    FILE_BYTE.remove(md5);
                                }
                                System.out.println("Send FILE_BYTES_RESPONSE to : " + socket.getInetAddress().toString()
                                        + ":" + socket.getPort());
                            }
                            break;
                        ...
                        case "FILE_BYTES_RESPONSE":
                            if (FILE_BYTETO.contains(md5)) {
                                String statusto = new TCPFileBytes(socket).FileBytes_request_continous(request, fileSystemManager);
                        ...
                        case "HANDSHAKE_REQUEST":
                            new TCPInvalidProtocol(socket).InvalidMessage("Connection already built.");
            ...
            if (TCPPeer.getConnectedPeers().contains(socket)) {
                TCPPeer.delConnectedPeers(socket);
            }
        } catch (IOException e) {
            e.printStackTrace();
            ...
            try {
                new TCPInvalidProtocol(socket).InvalidMessage("some mandatory information is missing");

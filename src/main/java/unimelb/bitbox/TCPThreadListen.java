...
public class TCPThreadListen extends Thread {
     ...
     */
    public TCPThreadListen(FileSystemManager fileSystemManager) {
    ...
    public void run() {
        ...
        try (ServerSocket serverSocket = factory.createServerSocket(port)) {
            ArrayList<TCPThreadServer> server = new ArrayList<>();
            int i = 0;
            // Wait for the connection
            while (true) {
                // Receive one connection
                new TCPThreadServer(serverSocket.accept(), fileSystemManager).start();

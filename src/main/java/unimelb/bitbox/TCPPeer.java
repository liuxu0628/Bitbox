...
public class TCPPeer {
    ...
    private static Logger log = Logger.getLogger(TCPPeer.class.getName());
    ...
    public static void main() throws IOException, NumberFormatException, NoSuchAlgorithmException {
        ...
        TCPThreadListen server = new TCPThreadListen(serverMain.fileSystemManager);
        server.start();
        new ThreadSyn(serverMain.fileSystemManager).start();
        new TCPThreadServeClient(serverMain.fileSystemManager).start();
        ...
        ArrayList<TCPThreadClient> client = new ArrayList<>();
        for (int i = 0; i < knownPeers.size(); i++) {
            try {
                Socket socket = new Socket(knownPeers.get(i).host, knownPeers.get(i).port);
                client.add(new TCPThreadClient(socket, serverMain.fileSystemManager));
                client.get(i).start();
